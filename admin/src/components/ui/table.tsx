...
function Table({ className, ...props }: React.ComponentProps<"table">) {
  return (
    <div
      ...
      className="relative w-full overflow-x-auto rounded-xl border border-border bg-card shadow-[0_2px_12px_rgba(61,57,52,0.06),inset_1px_1px_0_rgba(255,255,255,0.4)]"
    >
      <table
        data-slot="table"
...
function TableRow({ className, ...props }: React.ComponentProps<"tr">) {
  return (
    <tr
      data-slot="table-row"
      className={cn(
        "border-b border-border/80 transition-colors duration-[220ms] hover:bg-accent/50 data-[state=selected]:bg-muted",
        className
      )}
      {...props}
    />
